...
func CreateHash(rng io.Reader, password string, params *Params) (hash string, err error) {
	salt, err := generateRandomBytes(rng, params.SaltLength)
	if err != nil {
		return "", err
	}
...
func generateRandomBytes(rng io.Reader, n uint32) ([]byte, error) {
	b := make([]byte, n)
	_, err := rng.Read(b)
	if err != nil {
		return nil, err
