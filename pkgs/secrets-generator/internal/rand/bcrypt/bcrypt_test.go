...
func TestBcryptingIsEasy(t *testing.T) {
	...
	hp, err := GenerateFromPassword(rand.Reader, pass, 0)
	if err != nil {
...
func TestCostValidationInHash(t *testing.T) {
	...
	for c := 0; c < MinCost; c++ {
		p, _ := newFromPassword(rand.Reader, pass, c)
		if p.cost != DefaultCost {
			t.Errorf("newFromPassword should default costs below %d to %d, but was %d", MinCost, DefaultCost, p.cost)
		}
	}

	p, _ := newFromPassword(rand.Reader, pass, 14)
	if p.cost != 14 {
	...
	_, err := newFromPassword(rand.Reader, pass, 32)
	if err == nil {
...
func TestCostReturnsWithLeadingZeroes(t *testing.T) {
	hp, _ := newFromPassword(rand.Reader, []byte("abcdefgh"), 7)
	cost := hp.Hash()[4:7]
	expected := []byte("07$")
...
func BenchmarkEqual(b *testing.B) {
	...
	hash, _ := GenerateFromPassword(rand.Reader, passwd, DefaultCost)
	b.StartTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkDefaultCost(b *testing.B) {
	...
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		GenerateFromPassword(rand.Reader, passwd, DefaultCost)
	}
}
...
func TestPasswordTooLong(t *testing.T) {
	_, err := GenerateFromPassword(rand.Reader, make([]byte, 73), 1)
