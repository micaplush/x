...
func GenerateFromPassword(rng io.Reader, password []byte, cost int) ([]byte, error) {
	if len(password) > 72 {
		return nil, ErrPasswordTooLong
	}
	p, err := newFromPassword(rng, password, cost)
	if err != nil {
		return nil, err
	}
	return p.Hash(), nil
...
func newFromPassword(rng io.Reader, password []byte, cost int) (*hashed, error) {
	if cost < MinCost {
		cost = DefaultCost
	}
	p := new(hashed)
	...
	_, err = io.ReadFull(rng, unencodedSalt)
	if err != nil {
		return nil, err
	}
