	...
	actions := []NtfyAction{
		{
			...
			Url:    "googlechrome://navigate?url="+firstAlert.GeneratorURL, // Apparently we are not allowed to URL-encode these query parameters.
			Clear:  true,
		},
		{
			Action: "view",
			Label:  "Silence",
			Url:    "googlechrome://navigate?url="+firstAlert.SilenceURL,
			Clear:  false,
		},
	}
